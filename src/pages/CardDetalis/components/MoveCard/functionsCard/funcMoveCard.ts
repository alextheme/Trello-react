...
import { IBoardContent, ICardContent, IListContent } from '../../../../../common/interfaces/Interfaces';
...
export const moveCardOneList = ({ ...p }: IProps): INewCardPositon[] => {
  const { card, srcList, srcData, position } = p;
  ...
  cardsSort.splice(position - 1, 0, moveCard[0]);
...
export const moveCardBetweenLists = ({ ...p }: IProps): INewCardPositon[] => {
  const { card, srcList, destList, srcData, position } = p;
  const newCardPosition: INewCardPositon[] = [];
  ...
  sort.splice(position - 1, 0, moveCard[0]);
...
export const moveCardBetweenDesks = ({ ...p }: IProps): INewCardPositon[] => {
  const { destList, destData, position } = p;
  const newCardPosition: INewCardPositon[] = [];
  
  const cards = { ...destData.lists[destList].cards};

  if (position <= Object.keys(cards).length) {
